        ...
        context "after multiples moves" do
          it "returns checkmate status" do
            ...
            ["Pa1","Pb1","Pc1","Pd1","Pf1","Pg1",
              "Ph1","Pa2","Pb2","Pc2", "Pd2",
              "Pe2", "Pf2", "Pg2", "Ph2" ].each {|position| board.fill_square(position,nil)}

              board.fill_square("Ph1",pieces[:white_king])
              pieces[:white_king].current_position = "Ph1"

              board.fill_square("Pe3",pieces[:black_bishop_1])
              pieces[:black_bishop_1].current_position = "Pe3"
              board.fill_square("Pf3",pieces[:black_bishop_2])
              pieces[:black_bishop_2].current_position = "Pf3"
              board.fill_square("Ph3",pieces[:black_king])
              pieces[:black_king].current_position = "Ph3"
              expect(board.game_over?(pieces[:white_king],movements)).to eq("checkmate")
