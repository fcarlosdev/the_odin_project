  ...
  let(:pieces) {
    {
      ...
      black_king: get_piece(:black_pawn),
  ...
  describe '#move' do
    ...
    context "when is a valid ordinary move" do
      ...
      it "moves the king piece to the given position" do
        expect(move_king.move(board.get_piece("Kd4"),"Kd4","Kd5")).to eq(true)
    ...
    context "when is valid capture move" do

      it "allows the king to capture the opponent piece" do
        board.update_square("Kd5",pieces[:black_king])
        expect(move_king.move(board.get_piece("Kd4"),"Kd4","Kd5")).to eq(true)
        expect(board.get_piece("Kd4")).to be_nil
        expect(board.get_piece("Kd5")).to_not be_nil
      end
